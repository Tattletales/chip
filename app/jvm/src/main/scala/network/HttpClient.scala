...
trait HttpClient[F[_]] {
  def get[Response: EntityDecoder[F, ?]](uri: String): F[Response]
  ...
  def postAndIgnore[T: EntityEncoder[F, ?]](uri: String, body: T): F[Option[Unit]]

  def unsafePostAndIgnore[T: EntityEncoder[F, ?]](uri: String, body: T): F[Unit]
}
